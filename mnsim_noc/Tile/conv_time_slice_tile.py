    ...
    def set_tile_task(self, clock_num):
        ...
        if self.state == 0:
            ...
            if self.input_list:
                ...
                x_max = self.height_core + self.stride_core * (self.next_output[0] - 1) - self.padding_core
                x_req = min(self.height_input,x_max)
                y_max = self.width_core + self.stride_core * (self.next_output[1] - 1) - self.padding_core
                ...
                satisfy = True
                for x in range(0,self.height_core):
                    for y in range(0,self.width_core):
